...
module.exports = {
    ...
    devServer: {
        ...
        historyApiFallback: true, 力导向图.html
...
module.exports={
    ...
    mode: "production", // 为生产构建启用许多优化
    mode: "development", // 启用了有用的开发工具
...
entry: ["./app/entry1", "./app/entry2"],// object
entry: {// array
    a: "./app/entry-a",
    b: ["./app/entry-b1", "./app/entry-b2"]
},
// 默认路径为 ./src，这里应用程序开始执行，webpack 开始打包
```
...
output:{
    ...
    // 输出解析文件的目录，url 相对于 HTML 页面，对应使用filename里面的长效缓存项   
...
module: {
    ...
    rules: [
      ...
      {
      	// 这里是匹配条件，每个选项都接收一个正则表达式或字符串
      	// test 和 include 具有相同的作用，都是必须匹配选项，组合起来表示匹配指定目录下的指定后缀名文件
        test: /\.jsx?$/,
        include: [
          path.resolve(__dirname, "app")
        ],
...
```js
  devServer: {
    ...
    historyApiFallback: true, // 为true，对于index.html显示404，对象为多个路径
    hot: true, // 热模块更换。取决于HotModuleReplacementPlugin
    https: false, // 为true，对于自签名的证书颁发机构的对象
    noInfo: true, // only errors & warns on hot reload
    // ...
  },
```
